...
let testExamples = {
    ...
    'csharp':            { name: "CSharpExample",  command: "dotnet ./CSharpExample/bin/Release/net7.0/csharp-example.dll" },
    ...
    'playwrightCsharp':  { name: "PlayWrightCSharpExample",  command: "dotnet ./PlaywrightCSharp/bin/Release/net7.0/playwright-csharp-example.dll" },
    'seleniumCsharp': 	 { name: "SeleniumCSharpExample",  command: "dotnet ./seleniumCSharp/bin/Release/net7.0/selenium-csharp-example.dll" },
...
export function init(options, _programOpts) {
    ...
    if (exampleStr.includes('java')) {
        log.info('Make sure to run:  javac ');
    }
    log.info(`Writing config for ${Array.from(examples).join(',')} at ${outputPath}`);
