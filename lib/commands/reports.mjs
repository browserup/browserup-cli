...
export async function reports(args, programOpts) {
    ...
    const credentials = ClusterCredentialsRepository.getCredentials(programOpts, ["apiToken", "clusterUrl"]);
...
export async function uploadReports(options) {
    ...
    const reports = configRepo.config.reports();
    if (reports === undefined || reports.length === 0) {
        throw decoratedError("No reports defined")
    }
    const credentials = ClusterCredentialsRepository.getCredentials(options, ["apiToken", "clusterUrl"]);
    ...
    for (const report of reports) {
        ...
        overallResult = overallResult && result;
        log.info(`Uploaded Report: ${report.name}`);
    }
    log.info(
        `Report Upload: ${
...
export async function runReport(credentials, reportName, options) {
    ...
    try {
        const response = await WebConsoleClient.sendGetRequest({
            path: `/load/reports/report/${reportName}`,
            qParams: {
        ...
        throw decoratedError({msg: `Failed to get reports: ${e?.response?.data?.error || 'Unknown'}`, error: e})
    }
}
