...
export async function runReport(credentials, reportName, options) {
    ...
    try {
        const response = await WebConsoleClient.sendGetRequest({
            path: `/load/reports/studio/${reportName}`,
            qParams: {
        ...
        if (e.code === "ECONNREFUSED") {
            throw decoratedError({
                error: e,
                msg: `Unable to connect to BrowserUp Server at ${credentials.clusterUrl}`,
                type: ErrorType.CLUSTER_NOT_FOUND
            });
...
export async function createOrUpdateReport(credentials, report) {
    ...
    try {
        const response = await WebConsoleClient.sendPostRequest({
            qParams: {api_token: credentials.apiToken},
            path: '/load/reports/studio',
            data: payload,
            expectedCodes: [200]
        })
