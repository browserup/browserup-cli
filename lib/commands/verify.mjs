...
export async function verify(command, args, options) {
    ...
    return verifyInternal(result.repoAndTag, result.databank, command, artifactPath, options.verbose);
}
...
export function parseVerifyOptions(options) {
    ...
    if (databankFile) {
        try {
            ...
            [databank, databankStr] = DataBanksLoader.loadFile(
                databankFile,
                { headers: true },
