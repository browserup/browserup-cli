...
export async function verify(command, args, programOpts) {
    ...
    await verifyInternal(result.repoAndTag, result.databank, command, artifactPath, programOpts.verbose, args.keep);
...
export async function verifyInternal(repoAndTag, databank, command, artifactDir, verbose = false, keep = false) {
