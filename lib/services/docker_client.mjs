...
export class DockerClient {
    ...
    static  localDockerStatus() {
        try {
             this.dockerInfo();
            return "\nLocal Docker " + chalk.green("✓");
        } catch (error) {
            if (error.type === ErrorType.DOCKER_NOT_RUNNING) {
                return "\nLocal Docker " + chalk.red("-");
            } else if (error.type === ErrorType.DOCKER_NOT_INSTALLED) {
    ...
    static dockerInfo() {
        try {
            ...
            let cmd = isWindows ? 'docker info --format=\"{{json .}}\"' : 'docker info --format="{{json .}}"'

            let output = execSync(cmd, { encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'] });

            // Parse the output
            let info = JSON.parse(output);
            if (info.ServerErrors === undefined) {
                return info;
            } else {
                throw decoratedError({
                    msg: `ServerErrors ${info.ServerErrors}`,
            ...
            if (error.status === 127) {  // Command not found
                throw decoratedError({
                    ...
                    type: ErrorType.DOCKER_NOT_INSTALLED
                });
            }
