    ...
    static async waitForStack(stackName, stackId, region, waitStrategy = WAIT_FOR_STACK_CREATE) {
        ...
        try {
            await this.runWithRetries(waitStrategy, logStatus, async () => {
                ...
                throw decoratedError(`Stack is not ready yet, status: "${lastStackStatus}"`);
            });
        } catch (err) {
...
// ...
    ...
    static async waitForStacksToDestroy(region, stacks) {
        ...
        await this.runWithRetries(WAIT_FOR_STACK_DESTROY, logStatus, async () => {
            ...
            throw decoratedError("Stack is not deleted yet, it's in progress...");
        });
    }

    static getStackOutputParams(clusterStack, outputParams) {
        return Object.keys(outputParams).reduce((result, paramKey) => {
            const paramValue = clusterStack.outputs.find((o) => o.output_key === outputParams[paramKey]);
            result[paramKey] = paramValue ? paramValue.output_value : undefined;
            return result;
        }, {});
    }
}
