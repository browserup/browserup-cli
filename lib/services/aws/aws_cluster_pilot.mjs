...
export class AwsClusterPilot {
    ...
    async createCluster({ clusterParams, clusterName }) {
        ...
        try {
            ...
            const clusterStack = await this.createClusterStack(clusterName, clusterParams, clusterCredentials, secrets);
            ...
            const webconsoleUrl = `${secrets["BROWSERUP_WEBCONSOLE_PROTOCOL"]}://${clusterPublicIp}`;

            const grafanaClient = new GrafanaClient(
                webconsoleUrl,
                secrets['BROWSERUP_GRAFANA_PORT'],
                secrets['BROWSERUP_GRAFANA_USERNAME'],
                secrets['BROWSERUP_GRAFANA_PASSWORD']
            );

            await grafanaClient.createZookeeperDashboard();
            await grafanaClient.createGridServicesDashboard();
    ...
    async destroyMinionsNetworkStacks(clusterName) {
        const regions = await AwsEc2Client.getRegions();
        for (const region of regions) {
            await this.destroyMinionsNetworkStacksInRegion(clusterName, region);
        }
    }

    async destroyMinionsNetworkStacksInRegion(clusterName, region) {
        const allRegionStacks = await AwsCfClient.getAllStacks(region);
        const minionNetworkStacks = await AwsCfClient.getStacksByTagsFilters(allRegionStacks, {
            [ResourceTag.STACK_TYPE]: StackType.MINIONS_NETWORK_STACK,
            [ResourceTag.CLUSTER_NAME]: clusterName,
        });
    ...
    async destroyMinionsResources(clusterName, allRegionStacks, region) {
        ...
        log.debug("Destroying Minion ECS Stacks")
        await this.destroyMinionsStacks(clusterName);
        log.debug("Destroying Minion Update Browserup Stacks")
        await this.destroyMinionsUpdateBuClustersStacks(clusterName, allRegionStacks, region);
        log.debug("Destroying Minion Networks Stacks")
        await this.destroyMinionsNetworkStacks(clusterName);
    }
    ...
    async createClusterStack(clusterName, clusterParams, clusterCredentials, secrets) {
        try {
            ...
            const createdStack = await AwsCfClient.waitForStack({
                stackName: createdClusterStackInfo.stackName,
                stackId: createdClusterStackInfo.stackId,
                region: clusterParams.region
            });
    ...
    async requestCreateClusterStack(clusterName, clusterParams, secrets) {
        ...
        let wcPort = secrets["BROWSERUP_WEBCONSOLE_PROTOCOL"] === 'http' ? '80' : '443';
        let stackParams = {
            "PrivateDnsNamespaceName": secrets['BROWSERUP_PRIVATE_DNS_NAMESPACE_NAME'],
            "S3MinioIngressPort": secrets['BROWSERUP_S3_MINIO_HOST_PORT_1'],
            "WcIngressPort": wcPort,
            "GrafanaIngressPort": secrets['BROWSERUP_GRAFANA_PORT'],
    ...
    async createServiceStack(clusterName, clusterStackOutputs, clusterPublicIp, clusterParams, secrets) {
        try {
            ...
            await AwsCfClient.waitForStack({
                stackName: serviceStackInfo.stackName,
                stackId: serviceStackInfo.stackId,
                region: region
            });
        } catch (e) {
    ...
    async requestCreateServiceStack(clusterName, clusterStackOutputs, clusterPublicIp, clusterParams, secrets) {
        ...
        const stackParams = {
            ...
            "S3MinioSecretAccessKeyID": secrets["BROWSERUP_S3_MINIO_SECRET_ACCESS_KEY"],
            "S3MinioAccessKeyID": secrets["BROWSERUP_S3_MINIO_ACCESS_KEY_ID"],
            "BrowserupS3MinioPort1": secrets["BROWSERUP_S3_MINIO_PORT_1"],
            "BrowserupS3MinioPort2": secrets["BROWSERUP_S3_MINIO_PORT_2"],
            "S3MinioIngressPort": secrets["BROWSERUP_S3_MINIO_HOST_PORT_1"],
            "AllowSshAccessToMinionFromCidr": `${sshAccessToMinionFromIp}/32`,
            "WebconsoleProtocol": secrets["BROWSERUP_WEBCONSOLE_PROTOCOL"],
            "WcIngressPort": secrets["BROWSERUP_WEBCONSOLE_PROTOCOL"] === "http" ? "80" : "443",
            "GrafanaIngressPort": secrets["BROWSERUP_GRAFANA_PORT"],
