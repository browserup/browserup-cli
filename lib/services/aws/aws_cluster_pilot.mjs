...
export class AwsClusterPilot {
    ...
    async createCluster({ clusterParams, clusterName, useLocalCache }) {
        try {
            logWithSpinner("Creating cluster...");
            ...
            const clusterStack = createClusterStack(clusterName, clusterParams, secrets);
            const clusterStackOutputs = getClusterStackOutputs(clusterStack);
            const nginxInstance = await describeAsgInstance(clusterStackOutputs.nginxAsgId, clusterParams.region);
            const clusterPublicIp = nginxInstance.publicIpAddress;

            createServiceStack(clusterName, clusterStackOutputs, clusterPublicIp, clusterParams, secrets);
            ...
            logger.info(`Service stack for cluster '${clusterName}' has been created successfully.`);

            clusterCredentials.clusterUrl = webconsoleUrl;
            saveCredentials(clusterCredentials);

            return clusterCredentials;
        } catch (ex) {
            stopLogSpinner();
            logger.error(`Failed to initialize cluster: '${clusterName}', region: '${clusterParams.region}': '${ex.message}'`);
            logger.error("Any created stacks will be destroyed.");
            await destroyCluster({ clusterName, region: clusterParams.region });
            clearSecrets();
            throw ex;
        }
    }
