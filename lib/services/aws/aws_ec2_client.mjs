...
export class AwsEc2Client {
    static async availabilityZones(region) {
        ...
        log.debug(`Getting AZs from AWS for region ${region}`)
        const command = new DescribeAvailabilityZonesCommand(input);
        const azsResp = await ec2.send(command);
    ...
    static async findKeyPairs(keyNames, region) {
        const ec2 = new EC2({ region });
        try {
            ...
            throw decoratedError(`Failed to get key pairs: ${e.message} in region: ${region}`);
        }
    }
}
