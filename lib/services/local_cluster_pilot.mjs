...
export class LocalClusterPilot extends ClusterPilot {
    async createCluster({ clusterParams, clusterName }) {
        ...
        await DockerClient.dockerComposeUp({ services: allServices, dockerComposePath: composeYmlFile, env: envs });
    ...
    async destroyCluster({ clusterName, region, removeVolumes }) {
        const composeYmlFile = dockerComposeYmlPath();
        const secrets = ClusterSecretsProvider.get();
        await DockerClient.dockerComposeDown({ dockerComposePath: composeYmlFile, removeVolumes: removeVolumes, env: secrets });
    }
