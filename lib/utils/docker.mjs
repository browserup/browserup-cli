...
export class Docker {
  ...
  static async tag(theirImage, newTag) {
    ...
    if (errors.length) {
      throw new Error(`Failed to tag "${theirImage}" as "${newTag}". Caused by: \n>> ${errors.join(">> ")}`);
    }
  }
  ...
  static async build(dockerfile, tag, opts) {
    ...
    if (errors.length) {
      throw new Error(`Failed to build "${dockerfile}". Caused by: \n>> ${errors.join(">> ")}`);
    }
  }
  ...
  static async login(username, password, server) {
    ...
    if (exitCode > 0) {
      throw new Error(`Failed to login to registry. Caused by: \n>> ${errors.join(">> ")}`);
    }
  }
  ...
  static async push(tag) {
    ...
    const { exitCode, errors } = await Docker.execDockerCmd(cmd);
    if (errors.length) {
      throw new Error(`Failed to push "${tag}". Caused by: \n>> ${errors.join(">> ")}`);
    }
  }
  ...
  static async pull(tag) {
    ...
    const { exitCode, errors } = await Docker.execDockerCmd(cmd);
    if (errors.length) {
      throw new Error(`Failed to pull "${tag}". Caused by: \n>> ${errors.join(">> ")}`);
    }
  }
...
export function copyFilesToDotFolderIfMissing(servicesImageTag) {
  ...
  const externalUserSettingsFolder = LocalEnvVars.appSettingsPath("browserup");
  ...
  fs.copySync(internalResourcesPath, `${externalUserSettingsFolder}/`, { removeDestination: true });
  ...
  DcConfigEditor.updateServicesImageTags({
    dcYmlPath,
    serviceNames: ["grid-java-api", "grid-java-coordinator", "webconsole"],
    newImageTag: servicesImageTag
  });

  DcConfigEditor.updateEnvForService({
    dcYmlPath,
    serviceName: "grid-java-coordinator",
    envKey: "MINION_IMAGE_VERSION_TAG",
    envNewValue: servicesImageTag
  });
}
