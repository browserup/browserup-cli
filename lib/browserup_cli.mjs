...
export class BrowserUpCli {
    ...
    preFlight(name, args, opts){
        ...
        if (this.commandRequiresaConfig(name)) {
              if (opts.config != null) {
                  ...
                  console.debug(`Using config ${opts.config}`);
                  this.config = new Config(opts.config);
              }
        }
        copyFilesToDotFolderIfMissing(opts["servicesImageTag"] || SERVICES_VERSION)
    }

    commandRequiresaConfig(command) {
        return ["start", "stop", "deploy", "destroy", "upload_license"].includes(command);
    }
    ...
    async runCommand(func, args, programOptions, command = null) {
            try {
                ...
                if (func == null) {
                    ...
                    throw decoratedError({ error: e, type: ErrorType.INVALID_COMMAND, msg: msg })
    ...
    handleError(error) {
        ...
        if(error instanceof ExitProcessError) {
            // we already logged the error, then threw this for a soft exit for our tests,
            // so it doesn't blow up the test process
            log.debug('ExitProcessError');
            return false;
        }

        switch (error.type) {
            case ErrorType.CLUSTER_NEWER_THAN_CLI:
                return logAndExit("Cluster is newer than CLI, update CLI", error);
                break;
            case ErrorType.CLUSTER_OLDER_THAN_CLI:
            ...
            case ErrorType.UPLOAD_ERROR:
                ...
                break;
            case ErrorType.SCENARIO_PAYLOAD_ERROR:
                return logAndExit(`${error.message}`);
                break;
            case ErrorType.BROWSER_UP_NO_REPORTS_DEFINED:
                return logAndExit(`${error.message}`);
                break;
            case ErrorType.UPLOAD_SCENARIO_ERROR:
                return logAndExit(`${error.message}`);
                break;
            case ErrorType.NO_LAST_RUN_ID_ERROR:
                return logAndExit(`${error.message}`);
                break;
            case ErrorType.BROWSER_UP_CLUSTER_NOT_FOUND:
                return logAndExit(`${error.message}`);
                break;
            case ErrorType.BROWSER_UP_YAML_MISSING:
                return logAndExit(`${error.message}`);
                break;
            case ErrorType.COMMANDER_ERROR:
                return logAndExit(`${error.message}`);
                break;
            case ErrorType.MISSING_CLUSTER_CREDENTIALS:
                return logAndExit(`Error Uploading license ${error.message}`, error);
                break;
            case ErrorType.UPLOAD_LICENSE_ERROR:
                return logAndExit(`Error Uploading license ${error.message}`, error);
                break;
            case ErrorType.TYPE_ERROR_CONNECT:
                log.error("Connection Refused - Unable to Contact remote host");
                break;
            case ErrorType.INVALID_COMMAND_ERROR:
                return logAndExit(`Invalid command ${error.message}`, error);
                break;
            case ErrorType.ERROR:
                return logAndExit(`Failed ${error.message}`, error);
                break;
            case ErrorType.INVALID_CLUSTER_CREDENTIALS:
                return logAndExit("Could not find credentials for an active cluster");
                break;
            case ErrorType.INVALID_CONFIG_ERROR:
                return logAndExit("Invalid config file", error);
                break;
            case ErrorType.CLUSTER_ALREADY_EXISTS_ERROR:
                return logAndExit("Cluster exists - consider 'destroy' first to terminate current cluster", error);
                break;
            case ErrorType.EXIT_PROCESS_ERROR:
    ...
    constructor(exitOverride = false, outHsh = null) {
        ...
        this.program
            ...
            .addOption(new Option("-s, --services-image-tag <string>", "Dev only - override default image tag for grid-java-api, grid-coordinator, minion and webconsole", SERVICES_VERSION).hideHelp())
        ...
        cluster
            ...
            .option("-m, --cluster-type [string]", "Cluster type, either \"local\" or \"aws\".", "local")
            ...
            .option("-m, --minion-key-pair <string>", "Optional AWS Key Pair name to use for ECS minion instances")
            .action(async (args) => {
        ...
        load
            .command('verify')
            .argument("<command>", "Command to run in container. Defaults to 'echo hello world'")
            ...
            .option("-h, --show-har", "If show-har is passed, the captured traffic is output as a HAR file", false)
            .option("-a, --artifact-dir <string>", "Directory to copy into container at /home/browserup/artifact, which is also the working dir at container start", ".")
            ...
            .action(async (command,args, programOptions  ) => {
                this.runCommand(verify, args, this.program.opts(), command);}
            );
